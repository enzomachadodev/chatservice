...
import (
	...
	"github.com/m4chado/chatservice/internal/domain/entity"
	"github.com/m4chado/chatservice/internal/domain/gateway"
...
type ChatCompletionUseCase struct {
	ChatGatewat  gateway.ChatGateway
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	chat, err := uc.ChatGatewat.FindChatByID(ctx, input.ChatID)
	if err != nil {
		if err.Error() == "chat not found" {
			...
			//save on database***
			err = uc.ChatGatewat.CreateChat(ctx, chat)
			if err != nil {
	...
	err = uc.ChatGatewat.SaveChat(ctx, chat)
	if err != nil {
